...
| **macOS fixed TLS slots 108/109 → 96/97** | **Bun [`942b8342`](https://github.com/oven-sh/mimalloc/commit/942b8342) (`include/mimalloc/prim-tls.h:356-361`), MIT** | **5 — IMPORTED (#297)** | Not a hypothetical collision: per apple/darwin-libpthread's `tsd_private.h` (libpthread-454.40.3, `fe48dac35eb1232b0aa67260e79911387364568d`), 108/109 are `__PTK_FRAMEWORK_SWIFT_KEY8`/`_KEY9` — assigned, in-use slots. 96/97 sit in the never-assigned gap after `__PTK_FRAMEWORK_CORETEXT_KEY0`=95; Bun has production evidence for the choice. Not compile-exercised by any CI lane (macOS defaults to `MI_TLS_MODEL_PTHREADS`, not `MI_TLS_MODEL_FIXED`); verified with a local cross-build (`-DMI_TLS_MODEL_FIXED=ON` through soldr's Darwin toolchain) instead. CI now fetches apple/darwin-libpthread's `tsd_private.h` and fails if either slot is ever assigned. |
