...
| `MI_NO_PROCESS_DETACH` | Bun | **5 — open, scheduled (#268)** | Bun sets it unconditionally, so a rebase changes their process teardown behavior without it; the port is ~5 lines. Useful for embedders who own teardown, and it would have helped several exit-time crashes in this survey. Opt-in by nature. |
