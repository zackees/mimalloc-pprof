...
| `mi_page_map_get_idx` over-counts one slice for OS-aligned pages | Bun [`0e150b5c`](https://github.com/oven-sh/mimalloc/commit/0e150b5c) | **4 — not yet taken** | The two-level page-map variant indexes from `page_start` but still adds the `page_start - slice_start` correction the flat variant needs, so registration claims one slice past the page and unregistration zeroes it. Over-count reproduced by instrumentation (`corr=1 registered=2` for a 1 MiB-aligned request); the *corruption* — that slice landing on a live neighbour — is **not** reproduced. Only reachable via alignment > 64 KiB. Still unfixed at `upstream/dev3` tip. |
