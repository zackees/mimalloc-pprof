...
| `pthread_atfork` fork-safety handlers | Bun (`_mi_process_fork_prepare/parent/child`) | **5 on POSIX — IMPORTED (#270)** | Repro landed first (`test/test-fork-locks.c`): a churn thread holding `heaps_lock`/`arena_reserve_lock` racing 200 `fork()`s reproduced 0/200 failures at that exact CI-safe scale on this machine (the race window is narrow, matching Bun's own "Case A is probabilistic" caveat for the equivalent test), but a heavier ad hoc multi-threaded fork-storm stress harness (not part of the committed suite) produced multiple genuinely, permanently hung single-threaded child processes (confirmed via `/proc/<pid>/wchan`: `futex_do_wait`, thread count 1, never progressing) on the pre-#270 tree -- concrete evidence of the hazard. `src/subproc.c` now implements `_mi_process_fork_prepare/parent/child` (lock-order block + `mi_subproc_fork_prepare/parent/child`), registered once in `src/init.c`'s `mi_process_init_once` under `#if !defined(_WIN32) && !defined(__wasi__)`, and wired into `src/prim/osx/alloc-override-zone.c`'s zone `force_lock`/`force_unlock`/`reinit_lock` and its `_malloc_fork_prepare/parent/child` DYLD interpose (macOS: compile-verified only here, see #270's cross-CI note). Lock skeleton ported from Bun's `_mi_process_fork_prepare/parent/child`; `threadlocal.c`'s handler is a near-verbatim port. Bun's `tld` registry and scavenger/park state don't exist in this tree yet, so `mi_tld_t::theaps_lock` (per-thread, "sometimes accessed from another thread on `mi_heap_free`") is a **known, documented gap** left for the Phase 7 scavenger work (#264 item 7 / #272) -- marked `// Phase 7: scavenger` at every affected site. One thing this phase found that Bun's own design (as ported) does not handle: `mi_fork_depth` as a single process-wide atomic assumes fork() calls are serialized across threads, which glibc does **not** do (verified empirically -- concurrent `fork()` calls from different threads interleave their `pthread_atfork` handlers). The as-ported counter is only safe for same-thread nesting (macOS zone callback + `pthread_atfork` firing together for one `fork()`); under genuinely concurrent multi-threaded `fork()` it produced a real `internal_lock_release_by_non_owner` (an ABA race across overlapping fork "generations"), caught by the P1 reentrancy checker. Fixed here by making `mi_fork_depth` per-thread (`mi_decl_thread`) and adding `mi_fork_serialize_lock`, a real mutex giving true cross-thread exclusion around one fork generation. `MI_DEBUG` builds self-check the documented lock order (`mi_fork_lock_order_assert`, thread-local to avoid the same cross-thread hazard). Their documented ordering rule is kept verbatim: locks that can be held across an allocation are taken before `arena_reserve_lock`. |
...
| `mi_on_thread_idle` + background scavenger + hole purging (one feature) | Bun `src/scavenger.c`, `src/page.c` | **5 — open, scheduled (#272)** | `mi_on_thread_idle` is a linked Bun symbol whose contract spans all three, and Bun ships them default-on — losing them is a memory regression against bun#39844. The platform-native wait/wake is well built, but the profiler that walks per-page state now races with a background purger and the OS-page-keyed bitmap must stay consistent with our sample records — that interaction is designed in #272, not ported. Default-on, or the memory regression stands. |
...
| `mi_heap_dump_json` / `mi_heap_get_seq` | Bun | **5 — open, scheduled (#269)** | Backs the shipped `bun:jsc` `heapStats({dump:true\|"blocks"}).mimallocDump` API; `heapStats-mimalloc.test.ts` asserts the exact JSON shape (`heaps[].seq`, `pages[].{id,block_size,used,reserved,thread_id}`, `blocks[[id,size]]`). Rebasing Bun onto us deletes a shipped API without it. |
| Heap delete/destroy teardown race protocol | Bun (`src/theap.c` +346, `src/heap.c` +61) | **5 — open, scheduled (#271)** | `MimallocArena` does `mi_heap_new` per parse call and `mi_heap_destroy` on `Drop`, across threads, at high rate — the exact pattern their `test-heap-teardown.c` / `test-heap-churn.c` / `test-heap-aba.c` corpus stresses. We have none of it. |
| `stats.c` snapshot printing + counter-at-a-time `mi_stats_copy` | Bun | **4 — open, scheduled (#269)** | `_mi_stats_print` must print from a snapshot (heap and subproc stats update concurrently) and `mi_stats_copy` must copy counter-at-a-time to avoid torn reads; Bun has an open bug on this surface (bun#28630). |
...
| Windows PRNG / RAM-sizing / NUMA fixes | Bun `src/prim/windows/prim.c` (+78) | **3 — open, scheduled (#273)** | Small and self-contained, on a priority platform for both forks; audit-and-take rather than rediscover. |
| macOS malloc-zone introspection | Bun `alloc-override-zone.c` | **3 — open, scheduled (#273)** | In-/out-of-process `memory_reader_t` for `leaks`/`heap`/`vmmap` and fork-safe zone locking; a tooling regression on macOS without it. |
| macOS fixed TLS slots 96/97 | Bun `prim-tls.h` | **3 — open, scheduled (#273)** | 96/97 sit in the never-assigned gap after `__PTK_FRAMEWORK_CORETEXT_KEY0`=95; Bun has production evidence for the choice. A silent slot collision is a corruption class. |
...
| `purge_delay` default 1000 → 100 ms | Bun | **3 — open, scheduled (#272)** | Matches Bun's choice (bun#34217); lands with the scavenger, which it interacts with. Note: `arena_purge_mult` is **already 1** upstream — the earlier "10→1" claim in the Bun research was wrong; only `purge_delay` differs. |
