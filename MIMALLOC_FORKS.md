...
| `src/prof.c` (+719) | **pprof-compatible sampling heap profiler.** Zero fast-path cost when off; when on, `theap->prof_force_slow` poisons `pages_free_direct` so every malloc detours into `_mi_malloc_generic` where the byte countdown lives, and sampled blocks set a `MI_PAGE_HAS_PROF_SAMPLES` page flag so frees route to `_mi_prof_free`. Frame-pointer unwinding rather than `.eh_frame`; `build_id` in Mapping records; `<execinfo.h>` is only a *fallback*, guarded behind glibc/Apple because musl has no `backtrace()`. Not POSIX-only: Win32 uses `RtlCaptureStackBackTrace` (`src/prof.c:165`). Its Windows gap is mappings, not stacks — `mi_prof_collect_mappings`' `#else` is empty (`src/prof.c:563`), so Win32 profiles carry no module table and cannot be symbolized. |
...
| macOS | Fork-safe locking, fixed TLS slots → 96/97 (the never-assigned gap after `__PTK_FRAMEWORK_CORETEXT_KEY0`=95; ours too since #273 — upstream's 108/109 are `__PTK_FRAMEWORK_SWIFT_KEY8/9`, a confirmed collision), in- and out-of-process malloc-zone introspection via `memory_reader_t`. |
...
| Page hole purging (`purge_holes*`) | Bun `src/page.c` (+1038) | **5 — IMPORTED (#272, Phase 7b, PR #302)** | The second clause of `mi_on_thread_idle`'s contract: at an idle point, discard the memory of the free blocks sitting INSIDE a page that is still in use, in OS-page units, so one long-lived object no longer pins a whole 64 KiB/512 KiB page. Default-on (`mi_option_purge_holes`, appended after `scavenger`; no slot renumbered), with `purge_holes_eager_zero`, `purge_holes_min_interval` (100 ms) and `purge_holes_full_every` (64, Bun's value). The whole engine -- OS-page bitmap, sweep, unformed-tail discard, un-purge run/all, `swept_state` skip, sweep drivers, hole report -- is in a new `src/page-holes.c`; `src/page.c` carries exactly five hook calls (marked `HOOK n/5`) plus two debug-only assertion lines, and Bun's `src/theap.c` sweep drivers moved into the new file too (CLAUDE.md rule 6). `_mi_os_discard` / `_mi_prim_discard` on all platforms (MADV_DONTNEED / MADV_FREE_REUSABLE / MEM_RESET+VirtualUnlock) never touch commit state, because the arena tracks commit per 64 KiB slice and cannot represent a sub-slice hole. Public: `mi_purge_holes_stats_t` / `mi_purge_holes_stats_get` / `mi_purge_holes_report`, a `mi_stats_print` block, and `purge_holes_stats()` in the Rust crate. Measured on Bun's own workload shape (400k blocks, scattered survivors, idle between rounds): peak RSS 210 MB -> 105 MB. Alloc/free latency unchanged (the free path's only new code is inside `MI_CHECK_DOUBLE_FREE`; a freed block was live, hence never inside a discarded OS page), at the cost of `mi_page_t` growing 144 -> 192 bytes -- all of it at the tail, behind Bun's static assert that the hot fields still fit the first cache line. Two things the issue got wrong, both corrected here: `purge_holes_eager_zero` is a memzero-BEFORE-discard test-sharpening knob (it makes a mis-scoped discard corrupt visibly on macOS's lazy `MADV_FREE_REUSABLE`), not zero-tracking, so there is nothing to unify with `mi_option_purge_zeroes` -- which is in any case dead in this tree since the #80 pin bump (see #67) and is now documented as such in `mimalloc.h` rather than silently dropped. Profiler interaction designed and tested rather than assumed: a discard covers only free-block ranges and `_mi_prof_debug_assert_no_records_in` asserts per discard (MI_DEBUG) that no sampled record's block, and no record struct, lies inside it; `_mi_theap_area_visit_blocks` counts purged blocks as free so no inspection path can hand out a discarded pointer; nothing in the sweep touches `mi_tld_t.profiler`. Tests: `test-purge-holes.c` (1705 lines, 27 cases, imported and passing unmodified apart from the two file-reference adaptations) run twice (on / `MIMALLOC_PURGE_HOLES=0`), `test-purge-zero.c` verbatim, `test-park-handoff.c`'s two deferred `min_interval` cases enabled plus a new `-eager` variant, and a fifth scenario in `test-profile-race.c` that runs owner sweeps, scavenger sweeps and `mi_prof_visit`/`mi_theap_visit_blocks` against each other. |
| **`zalloc` skips `memset` after zero-purge (zero-tracking)** | **Bun [`b20b60d9`](https://github.com/oven-sh/mimalloc/commit/b20b60d9) `src/os.c` / `src/prim/*`, MIT** | **5 — IMPORTED (#79, lost in #80, restored by [#337](https://github.com/zackees/mimalloc-pprof/issues/337))** | `_mi_prim_decommit_zero` (Linux `MADV_DONTNEED`, Darwin `MADV_ZERO`; Windows never claims) → `_mi_os_purge_zero` → `mi_arena_purge` clears `slices_dirty` so the next `mi_zalloc` skips its memset. Gated on this fork's `mi_option_purge_zeroes` (=0; Bun has no knob). A release-build Linux regression guard in `test/bench-zero-tracking.c` fails when the feature is a no-op, which is how #80 slipped through. |
...
| **macOS malloc-zone introspection** | **Bun [`b20b60d9`](https://github.com/oven-sh/mimalloc/commit/b20b60d9) `src/prim/osx/alloc-override-zone.c`, MIT** | **5 — IMPORTED ([#349](https://github.com/zackees/mimalloc-pprof/pull/349), #339)** | `intro_enumerator` walks subproc → heaps → arena-pages bitmaps → pages → blocks through the caller's `memory_reader_t`, so `leaks`/`heap`/`vmmap`/`malloc_history` see the zone in-process and from another task; `intro_statistics` reports `mi_stats_get`; `zone->reserved1` is the root. #270's fork handling was kept over Bun's `mi_zone_locked_pid` variant. Tested by `test-osx-zone-introspect` (in-process) and `test-osx-zone-introspect-remote` (`task_for_pid` + `mach_vm_read_overwrite`, torn-link and unreadable-root cases), both executed by the selective macOS Recovery lane ([#348](https://github.com/zackees/mimalloc-pprof/pull/348)). |
...
| **Heap snapshot + `mi-heapview`** | **Bun [`b20b60d9`](https://github.com/oven-sh/mimalloc/commit/b20b60d9) `src/heap-snapshot.c`, `tools/mi-heapview.c`, MIT** | **5 — IMPORTED ([#338](https://github.com/zackees/mimalloc-pprof/issues/338))** | `mi_heap_snapshot` / `mi_heap_snapshot_to_file` / `MI_SNAPSHOT_BLOCKS`, `mi_option_snapshot_on_exit` (=0), the C viewer as the `mi-heapview` target, plus a Python reference reader and viewer in `examples/heap-snapshot/` whose output is pinned to the C tool's by `ci/tests/test_heap_snapshot_example.py`. Format version 1 is byte-identical to Bun's; the two non-format deviations are in the file header. |
