...
Written 2026-08-10 against a pipeline that has since changed. It was re-checked against
`main` before landing, and **two of its premises did not survive that check**. The document
is kept because its second problem is real and larger than it claimed; it is not an
approved plan, and the priority order below supersedes the Goals section.

**Premise 1 — "20–30 minutes, the dominant cost" — is stale.** Measured on
[run 34034422851](https://github.com/zackees/mimalloc-pprof/actions/runs/34034422851)
(benchmark-stats, 2026-09-06), the `build native allocator libraries` step ran
**6 m 15 s** inside a `build-and-measure` job that took **48 m 36 s** — about 13 %, and the
measurement phase is now the dominant cost, as it should be. Caching the allocator build is
therefore worth roughly six minutes a run, not twenty-five. That is worth having, but it no
longer justifies a shared-library/dlopen redesign on its own.

**Premise 2 — "adding an allocator touches five places" — understates it.** The natural
experiment is #325, the only allocator actually added since: `oven-sh/mimalloc` as a fifth
row touched **48 files, +3366/−472**, spread across `config.rs`, `latency.rs`,
`latency_runner.rs`, `memory.rs`, `memory_runner.rs`, `model.rs`, `orchestration.rs`,
`provenance.rs`, `runner.rs`, `scaling.rs`, `scaling_runner.rs`, `scenarios.rs`,
`validate.rs`, eight test files and the fixtures. The five dispatch points named below are
real and still present (`ADAPTER_SYMBOLS` and `COMPETITOR_SYMBOLS` at
`ci/build_benchmark_allocators.py:59,69`, `adapter_include_directories` at `:668`,
`find_primary_library` at `:810`, and `compile_adapter` at
`rust/benchmark-suite/build.rs:245`), but they are a minority of the real cost.

**What has already landed.** `rust/benchmark-suite/allocators/allocator-lock.json` exists
and carries each allocator's pin, archive URL, SHA-256, patches, build system and flags,
with `ci/build_benchmark_allocators.py --selftest` covering lock, checksum, traversal,
mode, source and link identity. That is the lockfile this document proposes, already in
place — as **static** archives, not the shared libraries described below. The
`upstream-mimalloc` pin moved to `6def7be9` in #332.

**Revised priority, if this is picked up.**

1. **The extensibility problem, measured properly first.** Before designing for it, work
   out how much of #325's 48 files a lockfile-plus-adapter shape would actually have
   removed. Much of that diff was allocator-count assumptions in the *statistics and
   validation* code, which no dispatch-table change touches.
2. **Caching**, on its own merits, as an `actions/cache` keyed on the lockfile hash — there
   is no `actions/cache` in any benchmark workflow today. This needs none of the ABI work.
3. **Shared libraries and `dlopen`** last, and only if 1 shows the static-archive shape is
   what blocks extensibility. It is the largest change here and the cost that motivated it
   has shrunk by 4x.

**Ordering note.** The benchmark publication pipeline is currently broken —
`benchmark-scaling.yml` has not published since 2026-08-14 (#376), tracked with the rest of
the chart work in #377. Adding machinery to a pipeline that cannot publish would be hard to
validate; fix that first.
