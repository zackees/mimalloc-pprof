...
| **dhat-off** (`dhat-off` `build` row in `run-linux`/`run-linux-serial`) | the **whole suite** with `MI_DHAT=OFF` (#371): the observer compiled out, so its per-allocation hook sites vanish from the alloc/free path and only the public `mi_dhat_*` API remains, as stubs. Every test is a test of those sites, which is why the whole suite runs rather than a smoke test | configure step greps the resolved defines for `MI_DHAT=0`, like `gated`/`guarded`; `test-dhat` and `test-fork-locks-dhat-env` are unregistered in this configuration by design |
...
| kind | rows | artifact |
|---|---|---|
| `bundle` | `release`, `pprof-off`, `debug-full`, `guarded`, `shared`, `gated` (#366, `MI_OWNER_GATE=ON`), `dhat-off` (#371, `MI_DHAT=OFF`), `musl`, `musl-pprof-off` | a `ci/bundle_tests.py` bundle, tarred, **plus** `show-only-<config>.json` straight from `ctest --show-only=json-v1` |
...
uv run ci/run_test_bundle.py \
  --bundles bundle/release bundle/pprof-off bundle/debug-full bundle/guarded bundle/shared bundle/gated bundle/dhat-off \
