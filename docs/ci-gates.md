...
| **opt-in defaults** (#414; `release`, `pprof-off`, `memevt-only`, `dhat-on` `build` rows, plus `-DMI_MEMEVT=ON -DMI_DIAGNOSTICS=ON` on `debug-full`/`musl`, `-DMI_DIAGNOSTICS=ON` on `gated`, both native `cl` trees, the `release`/`debug-full` bundles on Windows and macOS, and two `asan.yml` rows; the cargo lanes in `rust-native.yml`) | every observability subsystem is opt-in and OFF by default, so a row that names no flag tests the MINIMAL allocator. These rows are what keeps each subsystem tested: `release` = pprof+memory-events+diagnostics, `memevt-only` = memory-events with the profiler out, `dhat-on` = `MI_MEMEVT=0 MI_DHAT=1` (the one pair with a proven interaction -- DHAT dispatches through the memory-events slow paths, so its hook sites must survive), `debug-full` = both on together, `pprof-off` = everything out. Features are tested individually and all-on, never as a cross product (#307) | configure steps grep the resolved defines (`MI_DIAGNOSTICS=1` on `release`, `MI_MEMEVT=0` on `pprof-off`, `MI_MEMEVT=1` on `memevt-only`, `MI_DHAT=1` on `dhat-on`); the C tests of a compiled-out subsystem are not registered at all, so `ci/bundle_coverage.py`'s per-config enumeration is what notices a row losing them |
...
document). The same shape as the `guarded`/`gated`/`dhat-on` "assert on resolved
...
| kind | rows | artifact |
|---|---|---|
| `bundle` | `release` (#414: `MI_PPROF`+`MI_MEMEVT`+`MI_DIAGNOSTICS` all ON), `pprof-off` (#414: the MINIMAL build -- every subsystem OFF, spelled out), `memevt-only` (#414: `MI_MEMEVT=ON MI_PPROF=OFF`), `debug-full`, `guarded`, `shared`, `gated` (#366, `MI_OWNER_GATE=ON`), `dhat-on` (#371, `MI_DHAT=ON`; #414: deliberately WITHOUT `MI_MEMEVT`, the shape where the shared hook-site guard is load-bearing), `musl`, `musl-pprof-off` | a `ci/bundle_tests.py` bundle, tarred, **plus** `show-only-<config>.json` straight from `ctest --show-only=json-v1` |
...
uv run ci/run_test_bundle.py \
  --bundles bundle/release bundle/pprof-off bundle/memevt-only bundle/debug-full bundle/guarded bundle/shared bundle/gated bundle/dhat-on \
