...
| kind | rows | artifact |
|---|---|---|
| `bundle` | `release`, `pprof-off`, `debug-full`, `guarded`, `shared`, `gated` (#366, `MI_OWNER_GATE=ON`), `dhat-on` (#371, `MI_DHAT=ON`; DHAT is opt-in), `musl`, `musl-pprof-off` | a `ci/bundle_tests.py` bundle, tarred, **plus** `show-only-<config>.json` straight from `ctest --show-only=json-v1` |
...
uv run ci/run_test_bundle.py \
  --bundles bundle/release bundle/pprof-off bundle/debug-full bundle/guarded bundle/shared bundle/gated bundle/dhat-on \
