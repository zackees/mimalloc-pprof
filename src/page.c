...
static void mi_page_thread_collect_to_local(mi_page_t* page, mi_block_t* head)
{
  ...
  #if MI_PPROF
  // Uniform hook for both callers of this helper (mi_page_thread_free_collect and
...
static mi_theap_t* mi_malloc_generic_admin(mi_theap_t* theap) 
{
  ...
  if mi_unlikely(theap->generic_count >= 1000) {
    ...
    _mi_theap_purge_large_holes(theap, false); // #477: release large-page holes while busy
  }
  return theap;
}
